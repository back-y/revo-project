        ...
        <dt class="menu-item-handle">
            ...
            <span class="item-controls">
                ...
                <a class="item-edit" id="edit-<?php echo esc_attr( $item_id ); ?>"
                    title="<?php esc_attr_e('Edit Menu Item', 'revo'); ?>" href="<?php
    ...
    <div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr( $item_id ); ?>">
        ...
        <p class="field-description description description-wide">
            <label for="edit-menu-item-description-<?php echo esc_attr( $item_id ); ?>">
                ...
                <span
                    class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.','revo'); ?></span>
            </label>
        </p>
        <div class="advance-menu">
            <a href="javascript:void(0);" class="menu-advance-href"
                data-id="menu-advance-<?php  echo esc_attr( $item_id ) ?>"><?php esc_html_e( 'Show/Hide Advance Menu Config', 'revo'); ?>
            </a>
            <div class="menu-config-content" id="menu-advance-<?php  echo esc_attr( $item_id ) ?>"
                style="display: none;">
                <?php
						foreach (Revo_Menu_Admin::$custom_fields as $field => $field_opts):
                    ...
                    <label for="edit-menu-item-<?php echo esc_attr( $field ); ?>-<?php echo esc_attr( $item_id ); ?>">
                        ...
                        <?php
                            ...
                            class="code edit-menu-item-<?php echo esc_attr( $field ); ?>"
                            name="menu-item-<?php echo esc_attr( $field ); ?>[<?php echo esc_attr( $item_id ); ?>]"
                            value="<?php echo esc_attr( $item->$field ); ?>" /><?php
										break;
									case 'textarea':
										?><textarea id="edit-menu-item-<?php echo esc_attr( $field ); ?>-<?php echo esc_attr( $item_id ); ?>"
                            class="widefat edit-menu-item-<?php echo esc_attr( $field ); ?>" rows="3" cols="20"
                            name="menu-item-<?php echo esc_attr( $field ); ?>[<?php echo esc_attr( $item_id ); ?>]"><?php echo esc_html( $item->$field ); // textarea_escaped ?></textarea><?php
										break;
									case 'select':
										?><select id="edit-menu-item-<?php echo esc_attr( $field ); ?>-<?php echo esc_attr( $item_id ); ?>"
                            class="edit-menu-item-<?php echo esc_attr( $field ); ?>"
                            name="menu-item-<?php echo esc_attr( $field ); ?>[<?php echo esc_attr( $item_id ); ?>]">
                        ...
                        <?php
												echo '<img class="revo-opts-screenshot" id="revo-opts-screenshot-'.esc_attr( $item_id ).'" src="'. esc_url( ( $item->$field != '' ) ? esc_url( $item->$field ) : get_template_directory_uri() . '/lib/admin/img/placeholder.png' ) .'" width = "30" />';
                        ...
                        <?php endswitch; ?>
                        <?php if( $field_opts['type'] =='checkbox' ){ echo esc_html( $field_opts['label'] );} ?>
                        <?php if (isset( $field_opts['description'] ) && is_string( $field_opts['description'] )): ?><span
                            class="description"> ( <?php echo esc_html( $field_opts['description'] ); ?> )
                        </span><?php endif; ?>
                    </label>
                </p>
        ...
        <div class="menu-item-actions description-wide submitbox">
            ...
            <a class="item-delete submitdelete deletion" id="delete-<?php echo esc_attr( $item_id ); ?>" href="<?php
					...
					); ?>"><?php esc_html_e('Remove','revo'); ?></a> <span class="meta-sep"> | </span> <a
                class="item-cancel submitcancel" id="cancel-<?php echo esc_attr( $item_id ); ?>" href="<?php	echo esc_url( add_query_arg( array('edit-menu-item' => $item_id, 'cancel' => time()), remove_query_arg( $removed_args, admin_url( 'nav-menus.php' ) ) ) );
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $current_object_id = 0 ) {
		...
		if ( preg_match('/(apc|apcs)/i', $advanced) ){
			...
			if ( preg_match( '/apcs/i', $advanced ) && !empty( $page_content ) ){
				...
				if( defined( 'ELEMENTOR_VERSION' ) && \Elementor\Plugin::$instance->documents->get( $page_content )->is_built_with_elementor() ){
...
**/
get_template_part( 'lib/responsive_dropdown_sidebar' );